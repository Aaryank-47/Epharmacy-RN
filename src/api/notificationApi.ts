...
export const markAllNotificationsAsRead = async (): Promise<{ modifiedCount: number; matchedCount: number }> => {
  ...
  const unreadIds = unreadNotifications.notifications.map((log) => log._id);
...
export const getUnreadCount = async (): Promise<number> => {
  ...
  // Fallback to pagination.totalItems since we are filtering by isRead: false
  return (response as any).stats?.unreadLogs ?? response.pagination.totalItems ?? 0;
};
